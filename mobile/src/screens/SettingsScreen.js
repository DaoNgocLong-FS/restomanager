// =============================================================================
//  SettingsScreen — cấu hình API base URL (validate bằng react-hook-form)
// =============================================================================
...
// Regex URL hợp lệ: http(s)://host(:port) hoặc IP, có/không có path
...
export default function SettingsScreen({ navigation }) {
  ...
  const {
    ...
    defaultValues: { url: getApiBase() || 'http://192.168.1.10:3000/api' },
    mode: 'onSubmit',
  ...
  const onSubmit = async ({ url }) => {
    ...
    try {
      ...
      // Map lỗi kết nối thành field error trên URL để UX rõ hơn
      setError('url', { type: 'server', message: e.message || 'Không kết nối được' });
    ...
    <SafeAreaView edges={['bottom']} style={s.flex}>
      ...
      >
        <ScrollView style={s.flex} contentContainerStyle={s.wrap}>
          ...
          <Text style={s.hint}>
            Nhập địa chỉ API của backend RestoManager. Điện thoại và máy chủ phải cùng mạng Wi-Fi.
          </Text>
          ...
          <Controller
            ...
            name="url"
            rules={{
              required: 'Vui lòng nhập URL server',
              pattern:  { value: URL_RE, message: 'URL không hợp lệ (phải bắt đầu http:// hoặc https://)' },
              maxLength:{ value: 200,    message: 'URL quá dài' },
              validate: {
                hasHost: (v) => {
                  // Bắt buộc có host name / IP sau scheme
                  const m = String(v || '').match(/^https?:\/\/([^/]+)/i);
                  return (m && m[1].length >= 1) || 'URL thiếu địa chỉ host';
                },
              },
            }}
          ...
          />
          {errors.url ? (
            <Text style={s.fieldErr}>{errors.url.message}</Text>
          ) : (
            <Text style={s.helper}>
              Mặc định: <Text style={s.mono}>http://&lt;IP-LAN&gt;:3000/api</Text>
            </Text>
          )}
          ...
          >
            ...
            <Text style={s.btnText}>{isSubmitting ? 'Đang kiểm tra…' : 'Lưu & Kiểm tra'}</Text>
          </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  fieldErr: { alignSelf: 'flex-start', fontSize: 12, color: colors.error, marginTop: 6 },
